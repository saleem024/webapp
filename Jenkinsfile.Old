  ...
  stages {
     ...
     stage ('SAST') {
      ...
      stage ('Port Scan') {
		    steps {
		       	sh 'rm nmap* || true'
		       	sh 'docker run --rm -v "$(pwd)":/data uzyexe/nmap -sS -sV -oX nmap 54.234.14.90' 
			       sh 'cat nmap'
		    }
	        }
	...
	/*stage ('SSL Checks') {
		    steps {
		        sh 'docker run --rm -i nablac0d3/sslyze:5.0.0  54.234.14.90:8443 --json_out=results.json || true'
		    }
	    } */
    }
}
